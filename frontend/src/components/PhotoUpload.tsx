...
import { preparePhotoForUpload } from '../utils/preparePhotoForUpload'
import { IconCamera, IconGallery } from './Icons'

type UploadStatus = 'WAITING' | 'COMPRESSING' | 'UPLOADING' | 'PREPARING' | 'SUCCESS' | 'FAILED'
...
function statusLabel(item: QueueItem) {
  if (item.status === 'WAITING') return 'Hazırlanıyor'
  if (item.status === 'COMPRESSING') return 'Sıkıştırılıyor'
  if (item.status === 'UPLOADING') return 'Yükleniyor'
  if (item.status === 'PREPARING') return 'Yükleniyor'
  if (item.status === 'SUCCESS') return 'Tamamlandı'
  return item.error ?? 'Hata'
}
...
export function PhotoUpload({ onDone, onCancel, onBusyChange, stayOpenNotice = false }: Props) {
  ...
  const [galleryCta, setGalleryCta] = useState(false)
  ...
  async function uploadOne(item: QueueItem) {
    ...
    setQueue((current) =>
      current.map((entry) =>
        entry.id === id ? { ...entry, status: 'COMPRESSING', error: null } : entry,
      ),
    )
    ...
    try {
      ...
      const photo = await uploadPhotoWithRetry(prepared, id, {
        onPhase: (phase) => {
          if (phase === 'preparing') {
            setQueue((current) =>
              current.map((entry) =>
  ...
  async function uploadIds(items: QueueItem[]) {
    ...
    setQueue((current) => {
      ...
      if (failed === 0 && ok > 0) {
        setDone(
          ok === 1
            ? "Fotoğrafı başarıyla yüklediniz. Galeri'den eklediğiniz fotoğrafı görebilirsiniz!"
            : "Fotoğrafları başarıyla yüklediniz. Galeri'den eklediğiniz fotoğrafları görebilirsiniz!",
        )
        setGalleryCta(true)
      } else if (failed > 0) {
  ...
  return (
    <form className="upload-card" onSubmit={onSubmit}>
      <div className="upload-intro">
        ...
        <p>Bu özel günde çektiğiniz güzel kareleri seçin ve anı albümümüze ekleyin.</p>
      ...
      {!busy && done ? (
        <>
          ...
          {galleryCta ? (
            <Link className="btn btn--glass" to="/gallery" onClick={() => onDone?.()}>
